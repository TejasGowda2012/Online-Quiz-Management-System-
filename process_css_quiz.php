...
if (isset($_SESSION['username'])) {
    ...
    $userName = "Guest"; // Or handle the case where the user is not logged in
}
$resultDate = date('Y-m-d');
$subject = 'CSS'; // Subject is CSS

// Calculate marks
$marks = 0;
$sql = "SELECT * FROM mcqs WHERE category = 'CSS' AND id BETWEEN " . (($setNumber - 4) * 5 + 31) . " AND " . ($setNumber * 5 + 30); // Adjusted IDs for CSS
...
try {
    ...
    $mail->setFrom('ndrkquizportal@gmail.com', 'NDRK Quiz Platform');
    ...
    $mail->Body = "
        <h2>Quiz Results</h2>
        <p>Dear $username,</p>
        <p>Thank you for taking the CSS quiz. Here are your results:</p>
        <ul>
        ...
        <p>Best regards,<br> NDRK Quiz Platform Team</p>
    ";
